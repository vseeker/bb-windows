...
def getPDF(dictPdfFileAndPage,keyWords): # keyWords格式化处理后,即pdf文件名
    ...
    for k,v in dictPdfFileAndPage.items():
        pdfPath = getpath(k)
        pdfReader = PdfFileReader( open( pdfPath, "rb" ) )#读取PDF文件
        for el in v:
    ...
    outputStream = file("{0}".format(outputPdfPath),"wb")
...
def mergeQueryResult(keyWords = keyWordList): # 输入为语音识别后,然后经过简单分词以及停用词过滤后生成的关键词list
...
def queryResultParse(): # 解析查询到的结果以及经过合并形成的字典
    global dictQueryResult
    for k, v in dictQueryResult.items( ):
...
def generatePdf(k, v): # 先用字典dict存储v的分解结果,然后解析dict合并为pdf文档,并以k作为文件名
    dictPdfFileAndPage = {}
    page=[]
    for el in v:
        tmp = el.find( 'p' )
